...
- **Guard against `invalid` before every dereference.** BrightScript has no exceptions — a nil dereference aborts the channel. For example, `roHMAC.process()` returns `invalid` on an empty payload, and calling `.ToHexString()` on it would crash at the `Authorization` header. Always null-check external/computed values (`roHMAC` results, registry reads, JSON parses, response codes).
