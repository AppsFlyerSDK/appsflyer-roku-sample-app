...
```
PROJECT_CONTEXT:   AppsFlyer Roku SDK integration and sample app — reports first-open, session, and in-app events over HTTPS to AppsFlyer for a Roku (BrightScript) channel, and exposes conversion/attribution callbacks.
LANGUAGES:         BrightScript
NOTION_DB_URL:     NOTION_KEYWORDS:   JIRA_PROJECT_KEY:  ```
...
Project context: AppsFlyer Roku SDK integration and sample app — reports first-open, session, and in-app events over HTTPS to AppsFlyer for a Roku (BrightScript) channel, and exposes conversion/attribution callbacks.
Primary language(s): BrightScript
...
>   - Reply with the Notion database URL to enable enrichment. You can also add keywords to filter pages (e.g. `launch, attribution, session`) — if you don't, **`AppsFlyer Roku SDK integration and sample app — reports first-open, session, and in-app events over HTTPS to AppsFlyer for a Roku (BrightScript) channel, and exposes conversion/attribution callbacks.`** (the project name) will be used as the default filter.
...
Title format: `# AppsFlyer Roku Sample App — Feature Diagrams`
...
3. Filter pages whose title matches: 4. Before enriching, print a table of all meaningful Notion documents found: Title | Notion ID | Status | Likely enriches.
...
Tell the user: "Starting Jira enrichment (project: ) — reply 'skip' to skip." Wait for reply. If they skip, mark "Jira enrichment" as completed and end Phase 4B.

1. Proceed with enrichment.
2. Extract the seed ticket from `git branch --show-current`. Walk up to Epic and Initiative.
3. Also run keyword search across project  for each feature.
