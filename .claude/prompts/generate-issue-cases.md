...
- `AppsFlyer Roku SDK integration and sample app — reports first-open, session, and in-app events over HTTPS to AppsFlyer for a Roku (BrightScript) channel, and exposes conversion/attribution callbacks.` with the PROJECT_CONTEXT input value
- `BrightScript` with the LANGUAGES input value
